defmodule Mix.Tasks.Stac.BackfillTemporal do
  ...
  strings stored in `properties`.

  Rows written before the STAC temporal work (issue #20) can be inconsistent in
  three ways:
  ...
  is counted and reported, since those rows would now be refused on write.
  ...
  def run(args) do
    ...
    stats = process_all(batch_size, dry_run?)
    report(stats, dry_run?)
  end

  defp process_all(batch_size, dry_run?) do
    stream =
      Repo.stream(
        from(i in Item,
          select: %{
  ...
  defp report(stats, dry_run?) do
    verb = if dry_run?, do: "would change", else: "changed"
